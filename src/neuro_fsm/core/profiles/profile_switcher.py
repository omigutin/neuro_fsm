...
class StateProfilesSwitcher:
    ...
    def __init__(self, strategy: ProfileSwitcherStrategies = ProfileSwitcherStrategies.MIXED) -> None:
        self._strategy = strategy

    def choose_valid(self, profiles: ProfileTuple) -> Optional[Profile]:
        if self._strategy == ProfileSwitcherStrategies.BY_MATCH:
            return self._choose_by_match(profiles)
        elif self._strategy == ProfileSwitcherStrategies.BY_EXCLUSION:
            return self._choose_by_exclusion(profiles)
        elif self._strategy == ProfileSwitcherStrategies.MIXED:
    ...
    def _choose_by_match(self, profiles: ProfileTuple) -> Optional[Profile]:
        for profile in profiles:
            if profile.is_expected_seq_valid():
                return profile
        return None

    def _choose_by_exclusion(self, profiles: ProfileTuple) -> Optional[Profile]:
